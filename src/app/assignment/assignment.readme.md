...
`Assignment` (MongoDB collection): one document per service, with an embedded array of all historical assignment entries.

```typescript
// MongoDB document shape
type ServiceAssignmentDoc = {
  servId: number;           // unique — one document per service
...
**Source**: `src/app/assignment/api/route.ts` (`saveAssignments` handler)
...
**Used by**: `hydrateLastAssigned` (to populate `service.lastAssigned`)
...
**Used by**: `assignmentSelect` selectors for range queries
...
**Used by**: `assignment/api/route.ts` (`saveAssignments`)

---

## Redux State

The Redux slice holds a flat `docs: AssignmentDoc[]` array — all historical entries across all services, merged from API responses. The API flattens the embedded arrays before returning.

**Deduplication**: `getByServIds` uses `transformParams` to filter out already-loaded servIds before hashing, preventing duplicate API calls when streaming servIds arrive incrementally.

**Upsert**: New docs are merged into the flat array by appending (not replacing by servId), since multiple entries per servId are valid.
...
|----------|-------------|-----|
| `assignment/api/route.ts` | `isDuplicate` | Skip appending unchanged entries |
| `hydrateLastAssigned.ts` | `mostRecent` | Populate `service.lastAssigned` for scheduling display |
| `ServiceUtils.assignmentOutcome` | `service.lastAssigned` | Determine if a completed service satisfied its assignment |
...
| Feature | How assignments are loaded |
|---------|--------------------------|
| Cover Sheets | `useAssignments({ servIds })` — reactive on printed service servIds |
| Prenotify | `useAssignments({ servIds })` — reactive on printed service servIds |
| Daily Inventory (loadout page) | `assignmentActions.getBySchedDate` — for the selected route date |
| Daily Inventory (feedback) | `assignmentActions.getBySchedDate` — for the selected route date |
| Productivity | `assignmentActions.getBySchedDateRange` — for the selected date range |
| PaceCrawler | `useAssignments({ servIds })` — reactive on all active service servIds |
...
| `AssignmentModel.ts` | Mongoose model — `{ servId, assignments: AssignmentDoc[] }` |
| `assignmentSlice.ts` | Redux state — flat `docs: AssignmentDoc[]` array |
| `assignmentSelect.ts` | Selectors — all use `AssignmentUtils.canonical` for deduplication |
| `useAssignments.ts` | Hook — reactive per-servId loading + `saveAssignments` write function |
| `api/AssignmentContract.ts` | API contract |
| `api/route.ts` | API handlers — read (flatten arrays) + write (append-only) |
| `migrate/api/route.ts` | One-time migration from `ServiceDocProps.assignments` |
