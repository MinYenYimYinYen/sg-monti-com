...
- `src/app/priceIncrease/config/_lib/priceIncreaseConfigSlice.ts` — `IncreaseViewConfig` type; `viewConfig` actions (`setViewSortKeys`, `addViewSortKey`, `removeViewSortKey`, `toggleViewSortDirection`, `setViewGroupKey`, `setViewActiveGroup`, `resetViewConfig`)
...
- `src/app/priceIncrease/results/customerIncreaseResultsTypes.ts` — `CustomerIncreaseResult`, `SortableIncreaseProperties`, `GroupableIncreaseProperties`, `PreExistingFlagStatus`

**Sort/group registries**
- `src/app/priceIncrease/results/customerIncreaseSortFns.ts` — `customerIncreaseSortFns`, `customerIncreaseSortLabels`, `CustomerIncreaseSortKey`, `SortEntry`, `SortDirection`, `applySortDirection`
...
- `src/app/priceIncrease/usePriceIncreaseDeps.ts` — must call `useCustomerContext({ contexts: ["active"] })` to load customers into the active context. Without this, `centralSelect.customers` returns an empty array and all selectors produce no results. Also calls `usePriceIncreaseCustFlags()` to load custFlag data for all PI-module flag IDs.
- `src/app/priceIncrease/usePriceIncreaseCustFlags.ts` — loads custFlag data for `increaseFlagMappings`, `priceIncreaseExemptFlagId`, and `priceIncreaseManualFlagId`. Without this, `customer.flags` will be empty for these flagIds and `isExempt`, `isManual`, and `hasIncreaseFlag` will always be false.

**custFlag reconciliation (automatic after customer refresh)**
- `src/app/realGreen/custFlag/_lib/custFlagListeners.ts` — RTK listener that fires `refreshCustFlags` after any `refreshCustomer.fulfilled` action, keeping `customer.flags` in sync after a customer is refreshed.
...
5. Resolve the flag against `cappedPercent` → `resolvedFlag`
6. Determine `preExistingIncreaseFlags` from `customer.flags`
7. Derive `effectiveFlag` and `preExistingFlagStatus`
...
type IncreaseViewConfig = {
  sortKeys: SortEntry[];                 // ordered list of { key, direction } — primary first
...
type SortableIncreaseProperties = {
  ...
  increasePercent: number;  // effectiveFlag?.increasePercent ?? cappedPercent — reflects actual outcome
...
```typescript
type PreExistingFlagStatus = "none" | "matching" | "override" | "conflict";
```

- `"none"` — no recognized increase flag on the customer
- `"matching"` — one pre-existing flag that matches `resolvedFlag`
- `"override"` — one pre-existing flag that differs from `resolvedFlag` (pre-existing wins)
- `"conflict"` — multiple pre-existing increase flags, OR an increase flag alongside an exempt/manual flag

### `GroupableIncreaseProperties`

Boolean/categorical properties pre-computed for grouping. TypeScript enforces that every key has a corresponding entry in `customerIncreaseGroupFns`.
...
type GroupableIncreaseProperties = {
  ...
  isOverpriced: boolean;              // rawPercent < 0
  isBelowAcquisition: boolean;        // any service where nextPrice < acqPrice
  resolvedFlagDesc: string;           // effectiveFlag?.desc ?? "No Flag"
...
type CustomerIncreaseResult = {
  ...
  resolvedFlag: IncreaseFlag | null;         // module's computed flag
  preExistingIncreaseFlags: IncreaseFlag[];  // recognized increase flags already on the customer
  effectiveFlag: IncreaseFlag | null;        // flag that will actually be used (pre-existing overrides)
...
- Derives `preExistingIncreaseFlags` from `customer.flags` filtered against `increaseFlagIds`
- Derives `effectiveFlag`: pre-existing flag wins when present (single); null on conflict
- Derives `preExistingFlagStatus`: conflict when multiple flags, or increase flag + exempt/manual
...
`customerIncreaseSortFns: Record<keyof SortableIncreaseProperties, SortFn>` — all sort functions return descending comparator values. Use `applySortDirection(cmp, direction)` to flip for ascending.
...
`SortEntry = { key: CustomerIncreaseSortKey; direction: "asc" | "desc" }` — used in `IncreaseViewConfig.sortKeys`.

`applySortDirection(cmp, direction)` — negates `cmp` when `direction === "asc"`, no-op for `"desc"`.
...
Applies multi-sort (chained comparators in priority order, each with its own direction) then optional grouping. When grouped, defaults `activeGroup` to the first group if the stored value is null or stale.
...
- **Customer header**:
  - `CustomerLink` (opens RealGreen)
  - Exempt/manual flag description badges (destructive styling — actual flag names, not hardcoded text)
  - Pre-existing increase flag badges (primary styling; override = destructive border; conflict = destructive border + strikethrough)
  - Customer size
  - "Review" badge when `needsManualAttention`
  - Customer renewal revenue
  - Formula strip: Raw → [Bonus →] [Adjusted →] [Capped →] Flag [→ Override | → Conflict]
  - Non-target program code badges
  - Refresh button (calls `refreshCustomer(custId)` from `useActiveCustomers`; spins while refreshing)
...
Receives a single `ServiceIncreaseResult`. 7-column grid:
`servCodeId | size | acqPrice | nextPrice | planPrice | planDiff$ | planDiffPercent`

### `IncreaseViewControls.tsx`
Sort and group pickers. Reads from `priceIncreaseConfigSelect.viewConfig`, dispatches to `priceIncreaseConfigActions`.
- Sort picker: active keys shown as chips with label, ▼/▲ direction toggle, and × to remove. Popover shows all keys with priority number and direction toggle for active ones.
- Group picker: single-select with "None" option.
...
src/app/priceIncrease/results/
  ...
  customerIncreaseResultsTypes.ts         ← CustomerIncreaseResult, SortableIncreaseProperties, GroupableIncreaseProperties, PreExistingFlagStatus
  customerIncreaseResultsSelect.ts        ← customer-level aggregation selector
  customerIncreaseSortFns.ts              ← sort function registry (TypeScript-enforced completeness); SortEntry, SortDirection, applySortDirection
  ...
  _components/
    ...
    CustomerIncreaseCard.tsx              ← customer-level display card with refresh button
    ServiceIncreaseRow.tsx                ← per-service result row
    IncreaseViewControls.tsx              ← sort + group picker controls with direction toggle
...
src/app/priceIncrease/
  usePriceIncreaseCustFlags.ts            ← loads custFlag data for all PI-module flag IDs

src/app/realGreen/custFlag/_lib/
  custFlagListeners.ts                    ← RTK listener: fires refreshCustFlags after any refreshCustomer.fulfilled
