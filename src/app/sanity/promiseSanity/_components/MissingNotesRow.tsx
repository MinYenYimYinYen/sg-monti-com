...
export function MissingNotesRow({ missingNotesCustomer }: MissingNotesRowProps) {
  const { customer, services } = missingNotesCustomer;
      ...
      <div className="px-3 py-2 border-b border-border bg-accent/5">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-xs text-muted-foreground shrink-0">
            {services.length} service{services.length !== 1 ? "s" : ""} missing a valid promise note
          </span>
        </div>
        {customer.techNote && (
