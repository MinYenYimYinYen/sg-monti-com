...
import { MissingNotesCustomer } from "@/app/sanity/promiseSanity/promiseSanitySelect";
...
type ValidCasesRowProps = {
  validCasesCustomer: MissingNotesCustomer;
};
...
export function ValidCasesRow({ validCasesCustomer }: ValidCasesRowProps) {
  const { customer, services } = validCasesCustomer;
      ...
      <div className="px-3 py-2 border-b border-border bg-accent/5">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-xs text-muted-foreground shrink-0">
            {services.length} valid service{services.length !== 1 ? "s" : ""}
          </span>
        </div>
        {customer.techNote && (
          ...
          <div key={program.progId} className="px-3 py-2 space-y-1">
            ...
            {programServices.map((service) => (
              <div key={service.servId} className="flex items-start gap-2 text-xs pl-10">
                ...
                <span className="rounded px-1.5 py-0.5 font-medium bg-accent/10 text-accent shrink-0">
                  ✓ valid note
                </span>
                <span className="text-muted-foreground break-words">
                  {service.techNote || ""}
