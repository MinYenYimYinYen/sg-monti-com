...
export function ServiceCodeTable({ servCodes }: ServiceCodeTableProps) {
  const [expandedServCodeId, setExpandedServCodeId] = useState<string | null>(null);
  ...
  const toggleExpand = (servCodeId: string) => {
    setExpandedServCodeId(expandedServCodeId === servCodeId ? null : servCodeId);
  };
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-muted-foreground text-center">No service codes found</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Total Area (sqft)</TableHead>
              ...
              <React.Fragment key={servCodeData.servCodeId}>
                ...
                >
                  <TableCell className="font-medium">
                    {servCodeData.servCode.longName}
                  </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="bg-muted/20 p-4">
                      <div className="space-y-2">
                        <h4 className="font-semibold text-sm mb-2">Products Needed</h4>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-right py-1 px-2">Total Quantity</th>
                            ...
                            <tbody>
                              {servCodeData.products.map((product) => (
                                <tr key={product.productId} className="border-b">
                                  <td className="py-1 px-2">{product.productCommon.description}</td>
                                  <td className="py-1 px-2">{product.productCommon.productCode}</td>
                                  <td className="text-right py-1 px-2">
                                    {formatNumber(product.totalQuantity)}
                                  </td>
                                  <td className="py-1 px-2">{product.unitOfMeasure}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
