...
type ThunkConfig = {
  ...
  cacheDuration?: number; // Time-to-live in ms (e.g., 5000)
};
```
...
**File:** `src/lib/api/smartThunkOptions.ts`
...
* **Caching:** If `cacheDuration` is set and the data is fresh (checked against `lastFetched`), the request is cancelled.
...
2.  **`WithConfig<T>`**: Wraps the params to allow `force` and `cacheDuration` to be passed in. Without this, TypeScript will reject your smart options.
3.  **`AppState`**: Required in the third generic slot so `getState` works correctly in the condition.

```typescript
import { createAsyncThunk } from "@reduxjs/toolkit";
import { WithConfig } from "@/store/reduxUtil/reduxTypes";
import { smartThunkOptions } from "@/lib/api/smartThunkOptions";
...
>(
  ...
  async (params, { rejectWithValue }) => {
    ...
    const { force, cacheDuration, showLoading, ...apiParams } = params;
...
**Important:** The `WithConfig` options (`force`, `cacheDuration`, `showLoading`) **ONLY** work with `createAsyncThunk`.
...
2.  **Concurrency Rule:** Is this thunk type already in `activeRequests`? -> **CANCEL**.
3.  **Cache Rule:**
    * Is `cacheDuration` > 0?
    * AND is `(Now - lastFetched) < cacheDuration`? -> **CANCEL**.
4.  **Default:** -> **RUN**.